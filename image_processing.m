...
for k = 0:num_rows-1 
  n = n+1;  
        eval(['I1 = double(imread(''Meat_' num2str(depth) '_' num2str(j) '_' num2str(k) '_1.tif''));'])
        eval(['I2 = double(imread(''Meat_' num2str(depth) '_' num2str(j) '_' num2str(k) '_2.tif''));'])
        eval(['I3 = double(imread(''Meat_' num2str(depth) '_' num2str(j) '_' num2str(k) '_3.tif''));'])
        eval(['sectioned = double(imread(''Meat_' num2str(depth) '_' num2str(j) '_' num2str(k) '.tif''));'])
...
%         I3 =rescale(I3);
      ...
      %    eval(['imwrite(uniform,''20X_uniform_' num2str(n) '.tif'');']) 
          eval(['imwrite(pair,''/result/pair_' num2str(n) '.png'');']) 
